...
tsireal_end_time <- now()
tsireal_end_time-tsireal_start_time


train_tsireal <- 
    predict(tsireal_model
            ,newdata = 
              train_data_1 %>% 
              drop_data_underc %>% 
              select(
...
```{r}
length(train_nmf)
length(test_nmf);nrow(test_data_1)
```
...
train_data_1_dateid <- 
    ...
    # 重塑date_id
    rename(y = p) %>% 
    mutate(ds = as.Date('2018-01-01')+ddays(1*row_number()))

train_data_prophet <- 
    train_data_1_dateid %>% 
    select(short_name,ds,y) %>%
    arrange(ds) %>%
...
```{r eval=F}
prophet_pred_data %>% 
    mutate(train_data = map2(.x = train_n,.y = pred,.f = ~.y %>% head(.x))) %>% 
    # mutate(test_data = map2(.x = n,.y = pred,.f = ~.y %>% tail(.x)))
    select(short_name,data,train_data) %>% 
    unnest() %>% 
    select(short_name,ds,yhat) %>% 
    right_join(
        train_data_1_dateid %>% 
            select(short_name,ds,t)
        ,by = c('short_name','ds')
    ) %>% 
    select(-ds) %>% 
    # anyNA() %>% 
    ...
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_feature_prophet_train.csv',sep='_')
        )
    )    
```
...
prophet_pred_data %>% 
    mutate(test_data = map2(.x = n,.y = pred,.f = ~.y %>% tail(.x))) %>% 
    select(short_name,test_data) %>% 
    unnest() %>% 
    arrange(short_name,ds) %>% 
    select(short_name,yhat) %>% 
    bind_cols(
        test_data_1 %>% 
            select(short_name,t) %>% 
    ...
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_feature_prophet_test.csv',sep='_')
        )
    )    
```
...
train_prophet <- 
    fread(file.path(
        'data'
        ,list.files('data') %>% str_subset('jiaxiang_feature_prophet_train.csv') %>% max
    )
    ,encoding = 'UTF-8'
    )
test_prophet <- 
    fread(file.path(
        'data'
        ,list.files('data') %>% str_subset('jiaxiang_feature_prophet_test.csv') %>% max
    )
    ,encoding = 'UTF-8'
    )
```
...
需要重塑date_id，bug见
[Github Issue](https://github.com/facebook/prophet/issues/744)

```{r}
fread('prophet_date_id_bug.csv')
```
...
## baseline xgboost tweedie delete underc_meanhour data daily data y and x add 15 min data parmas
...
find_rmse %>% 
    # filter(valid_rmse == min(valid_rmse))
    arrange(abs(train_mae-valid_mae))
    # arrange(valid_mae)
```
...
```{r eval=F}
valid_data03.0underc_meanhour_train <- 
    valid_data03.0underc_meanhour %>% 
    ungroup %>% 
    # unless
    # Error in mutate_impl(.data, dots) : Column `predicition` must be length 11808 (the group size) or one, not 46571
    select(id,t,p) %>% 
...
add2evaluation::phv_metric(
    valid_data03.0underc_meanhour_train$short_name
    ,valid_data03.0underc_meanhour_train$t
    ,valid_data03.0underc_meanhour_train$p
    ,valid_data03.0underc_meanhour_train$predicition)
```
...
## baseline xgboost tweedie delete underc data add pca 1

```{r eval=F}
train_data_3.0_underc_addpca <- 
    train_data_3.0 %>% 
    drop_data_underc %>% 
    ungroup() %>% 
...
```{r eval=F}
set.seed(123)
split <- sample(nrow(train_data_3.0_underc_addpca),nrow(train_data_3.0_underc_addpca)*0.8,replace = T)
train_data03.0underc_addpca <- train_data_3.0_underc_addpca[split,]
valid_data03.0underc_addpca <- train_data_3.0_underc_addpca[-split,]
dtrain03.0underc_addpca <- xgb.DMatrix(data = as.matrix(train_data03.0underc_addpca %>% select(-t,-tsi_real,-p)),
                      label = train_data03.0underc_addpca$p+100
                      # gamma 回归 y > 0
                        )
dvalid03.0underc_addpca <- xgb.DMatrix(data = as.matrix(valid_data03.0underc_addpca %>% select(-t,-tsi_real,-p)),
                      label = valid_data03.0underc_addpca$p+100
                        )
watchlist03.0underc_addpca <- list(train=dtrain03.0underc_addpca,
                  valid=dvalid03.0underc_addpca
                  )
```
...
library(lubridate)
xgb.DMatrix.save(
    dtrain03.0underc_addpca
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dtrain03.0underc_addpca.buffer',sep='_')
    )
)
xgb.DMatrix.save(
    dvalid03.0underc_addpca
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dvalid03.0underc_addpca.buffer',sep='_')
    )
)
```
...
dtrain03.0underc_addpca <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dtrain03.0underc_addpca.buffer') %>% max
        )
    )
dvalid03.0underc_addpca <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dvalid03.0underc_addpca.buffer') %>% max
        )
    )
```
...
xgb_hyper_grid <- 
    function(eta,nround,max_depth,min_child_weight,gamma,subsample,colsample_bytree,nfold){
        ...
        xgb <- xgb.train(
         ...
         ## 其他
           seed = 123,
           watchlist=watchlist03.0underc_addpca,
           nfold = nfold,
           early_stopping_rounds = 50,
           nthread = 8
           )
...
set.seed(123)
start_time <- now()
xgb_base05underc_addpca <- xgb.train(
    data=dtrain03.0underc_addpca,
    ## 1
    eta = 0.3,
    nround=200,
...
end_time <- now()
get_log_xgb(xgb_base05underc_addpca,start_time,end_time)
```

```{r eval=F}
xgb.save(
    xgb_base05underc_addpca
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'xgb_base05underc_addpca.model',sep='_')
    )
)
```
...
xgb_base05underc_addpca <- 
    xgb.load(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('xgb_base05underc_addpca.model') %>% max
        )
    )
```
...
```{r eval=F}
setdiff(
    train_data_3.0_underc_addpca %>% select(-t,-tsi_real,-p) %>% names
    ,test_data_3.0 %>% select(-id,-t) %>% names
)
```
...
test_data_3.0 %>% 
    ...
    mutate(
        predicition = 
            ...
            mutate(pc1 = test_pca) %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca,.) %>% 
            `-`(100)
            # length()
            # 46571
            # round(.,1)
...
```{r eval=F}
xgb.importance(feature_names = colnames(dtrain03.0underc_addpca), model = xgb_base05underc_addpca) %>% 
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_imp_tbl.csv',sep='_')
...
```{r eval=F}
valid_data03.0underc_addpca_train <- 
    valid_data03.0underc_addpca %>% 
    ungroup %>% 
    # unless
    # Error in mutate_impl(.data, dots) : Column `predicition` must be length 11808 (the group size) or one, not 46571
    select(id,t,p) %>% 
    mutate(
        predicition = 
            ...
            valid_data03.0underc_addpca %>% 
            select(
                colnames(dtrain03.0underc_addpca)
                ) %>% 
            # setdiff(
            #     colnames(dtrain03.0underc_addpca)
            #     ,colnames(valid_data03.0underc_addpca)
            # )
            ungroup() %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca,.) %>% 
...
add2evaluation::phv_metric(
    valid_data03.0underc_addpca_train$short_name
    ,valid_data03.0underc_addpca_train$t
    ,valid_data03.0underc_addpca_train$p
    ,valid_data03.0underc_addpca_train$predicition)
```
...
## baseline xgboost tweedie delete underc data add pca 1 add tsi_real

```{r eval=F}
train_data_3.0_underc_addpca_addtsireal <- 
    train_data_3.0 %>% 
    drop_data_underc %>% 
    ungroup() %>%
    mutate(pc1 = train_pca) %>% 
    mutate(tsi_real = train_tsireal)
```
...
```{r eval=F}
set.seed(123)
split <- sample(nrow(train_data_3.0_underc_addpca_addtsireal),nrow(train_data_3.0_underc_addpca_addtsireal)*0.8,replace = T)
train_data03.0underc_addpca_addtsireal <- train_data_3.0_underc_addpca_addtsireal[split,]
valid_data03.0underc_addpca_addtsireal <- train_data_3.0_underc_addpca_addtsireal[-split,]
dtrain03.0underc_addpca_addtsireal <- xgb.DMatrix(data = as.matrix(train_data03.0underc_addpca_addtsireal %>% select(-t,-p)),
                      label = train_data03.0underc_addpca_addtsireal$p+100
                      # gamma 回归 y > 0
                        )
dvalid03.0underc_addpca_addtsireal <- xgb.DMatrix(data = as.matrix(valid_data03.0underc_addpca_addtsireal %>% select(-t,-p)),
                      label = valid_data03.0underc_addpca_addtsireal$p+100
                        )
watchlist03.0underc_addpca_addtsireal <- list(train=dtrain03.0underc_addpca_addtsireal,
                  valid=dvalid03.0underc_addpca_addtsireal
                  )
```
...
library(lubridate)
xgb.DMatrix.save(
    dtrain03.0underc_addpca_addtsireal
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dtrain03.0underc_addpca_addtsireal.buffer',sep='_')
    )
)
xgb.DMatrix.save(
    dvalid03.0underc_addpca_addtsireal
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dvalid03.0underc_addpca_addtsireal.buffer',sep='_')
    )
)
```
...
dtrain03.0underc_addpca_addtsireal <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dtrain03.0underc_addpca_addtsireal.buffer') %>% max
        )
    )
dvalid03.0underc_addpca_addtsireal <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dvalid03.0underc_addpca_addtsireal.buffer') %>% max
        )
    )
```
...
xgb_hyper_grid <- 
    function(eta,nround,max_depth,min_child_weight,gamma,subsample,colsample_bytree,nfold){
        set.seed(123)
        xgb <- xgb.train(
         data=dtrain03.0underc_addpca_addtsireal,
         ## 1
           eta = eta,
           nround=nround,
         ## 2
         ...
         ## 其他
           seed = 123,
           watchlist=watchlist03.0underc_addpca_addtsireal,
           nfold = nfold,
           early_stopping_rounds = 50,
           nthread = 8
           )
...
set.seed(123)
start_time <- now()
xgb_base05underc_addpca_addtsireal <- xgb.train(
    data=dtrain03.0underc_addpca_addtsireal,
    ## 1
    eta = 0.3,
    nround=200,
...
end_time <- now()
get_log_xgb(xgb_base05underc_addpca_addtsireal,start_time,end_time)
```

```{r eval=F}
xgb.save(
    xgb_base05underc_addpca_addtsireal
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'xgb_base05underc_addpca_addtsireal.model',sep='_')
    )
)
```
...
xgb_base05underc_addpca_addtsireal <- 
    xgb.load(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('xgb_base05underc_addpca_addtsireal.model') %>% max
        )
    )
```
...
test_data_3.0 %>% 
    ...
    mutate(
        predicition = 
            ...
            ungroup() %>% 
            mutate(pc1 = test_pca) %>% 
            mutate(tsi_real = test_tsireal) %>%             
            select(colnames(train_data03.0underc_addpca_addtsireal %>% select(-t,-p))) %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca_addtsireal,.) %>%
            # predict(xgb_base05underc_addpca_addtsireal
            #         ,newdata = train_data03.0underc_addpca_addtsireal %>% 
            #             select(-t,-p) %>% 
            #             as.matrix()
            #             )
            `-`(100)
...
```{r eval=F}
xgb.importance(feature_names = colnames(dtrain03.0underc_addpca_addtsireal), model = xgb_base05underc_addpca_addtsireal) %>% 
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_imp_tbl.csv',sep='_')
...
```{r eval=F}
valid_data03.0underc_addpca_addtsireal_train <- 
    valid_data03.0underc_addpca_addtsireal %>% 
    ungroup %>% 
    # unless
    # Error in mutate_impl(.data, dots) : Column `predicition` must be length 11808 (the group size) or one, not 46571
    select(id,t,p) %>% 
    mutate(
        predicition = 
            ...
            valid_data03.0underc_addpca_addtsireal %>% 
            select(
                colnames(dtrain03.0underc_addpca_addtsireal)
                ) %>% 
            # setdiff(
            #     colnames(dtrain03.0underc_addpca_addtsireal)
            #     ,colnames(valid_data03.0underc_addpca_addtsireal)
            # )
            ungroup() %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca_addtsireal,.) %>% 
...
add2evaluation::phv_metric(
    valid_data03.0underc_addpca_addtsireal_train$short_name
    ,valid_data03.0underc_addpca_addtsireal_train$t
    ,valid_data03.0underc_addpca_addtsireal_train$p
    ,valid_data03.0underc_addpca_addtsireal_train$predicition)
```


## baseline xgboost tweedie delete underc data add pca 1-2

```{r eval=F}
train_data_3.0_underc_addpca2 <- 
    train_data_3.0 %>% 
    drop_data_underc %>% 
    ungroup() %>% 
    bind_cols(
...
```{r eval=F}
set.seed(123)
split <- sample(nrow(train_data_3.0_underc_addpca2),nrow(train_data_3.0_underc_addpca2)*0.8,replace = T)
train_data03.0underc_addpca2 <- train_data_3.0_underc_addpca2[split,]
valid_data03.0underc_addpca2 <- train_data_3.0_underc_addpca2[-split,]
dtrain03.0underc_addpca2 <- xgb.DMatrix(data = as.matrix(train_data03.0underc_addpca2 %>% select(-t,-tsi_real,-p)),
                      label = train_data03.0underc_addpca2$p+100
                      # gamma 回归 y > 0
                        )
dvalid03.0underc_addpca2 <- xgb.DMatrix(data = as.matrix(valid_data03.0underc_addpca2 %>% select(-t,-tsi_real,-p)),
                      label = valid_data03.0underc_addpca2$p+100
                        )
watchlist03.0underc_addpca2 <- list(train=dtrain03.0underc_addpca2,
                  valid=dvalid03.0underc_addpca2
                  )
```
...
library(lubridate)
xgb.DMatrix.save(
    dtrain03.0underc_addpca2
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dtrain03.0underc_addpca2.buffer',sep='_')
    )
)
xgb.DMatrix.save(
    dvalid03.0underc_addpca2
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dvalid03.0underc_addpca2.buffer',sep='_')
    )
)
```
...
dtrain03.0underc_addpca2 <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dtrain03.0underc_addpca2.buffer') %>% max
        )
    )
dvalid03.0underc_addpca2 <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dvalid03.0underc_addpca2.buffer') %>% max
        )
    )
```
...
xgb_hyper_grid <- 
    function(eta,nround,max_depth,min_child_weight,gamma,subsample,colsample_bytree,nfold){
        ...
        xgb <- xgb.train(
         ...
         ## 其他
           seed = 123,
           watchlist=watchlist03.0underc_addpca2,
           nfold = nfold,
           early_stopping_rounds = 50,
           nthread = 8
           )
...
set.seed(123)
start_time <- now()
xgb_base05underc_addpca2 <- xgb.train(
    data=dtrain03.0underc_addpca2,
    ## 1
    eta = 0.3,
    nround=200,
...
end_time <- now()
get_log_xgb(xgb_base05underc_addpca2,start_time,end_time)
```

```{r eval=F}
xgb.save(
    xgb_base05underc_addpca2
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'xgb_base05underc_addpca2.model',sep='_')
    )
)
```
...
xgb_base05underc_addpca2 <- 
    xgb.load(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('xgb_base05underc_addpca2.model') %>% max
        )
    )
```
...
```{r eval=F}
setdiff(
    train_data_3.0_underc_addpca2 %>% select(-t,-tsi_real,-p) %>% names
    ,test_data_3.0 %>% select(-id,-t) %>% names
)
```
...
test_data_3.0 %>% 
    ...
    mutate(
        predicition = 
            ...
            as.matrix %>% 
            predict(xgb_base05underc_addpca2,.) %>% 
            `-`(100)
            # length()
            # 46571
            # round(.,1)
...
```{r eval=F}
xgb.importance(feature_names = colnames(dtrain03.0underc_addpca2), model = xgb_base05underc_addpca2) %>% 
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_imp_tbl.csv',sep='_')
...
```{r eval=F}
valid_data03.0underc_addpca2_train <- 
    valid_data03.0underc_addpca2 %>% 
    ungroup %>% 
    # unless
    # Error in mutate_impl(.data, dots) : Column `predicition` must be length 11808 (the group size) or one, not 46571
    select(id,t,p) %>% 
    mutate(
        predicition = 
            ...
            valid_data03.0underc_addpca2 %>% 
            select(
                colnames(dtrain03.0underc_addpca2)
                ) %>% 
            # setdiff(
            #     colnames(dtrain03.0underc_addpca2)
            #     ,colnames(valid_data03.0underc_addpca2)
            # )
            ungroup() %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca2,.) %>% 
...
add2evaluation::phv_metric(
    valid_data03.0underc_addpca2_train$short_name
    ,valid_data03.0underc_addpca2_train$t
    ,valid_data03.0underc_addpca2_train$p
    ,valid_data03.0underc_addpca2_train$predicition)
```
...
## baseline xgboost tweedie delete underc data add pca 1 add nmf

```{r eval=F}
train_data_3.0_underc_addpca_addnmf <- 
    train_data_3.0 %>% 
    drop_data_underc %>% 
    ungroup() %>%
    mutate(pc1 = train_pca) %>% 
    mutate(nmf = train_nmf) %>% 
    select(-tsi_real)
```

```{r}
train_data_3.0_underc_addpca_addnmf %>% names
```
...
```{r eval=F}
set.seed(123)
split <- sample(nrow(train_data_3.0_underc_addpca_addnmf),nrow(train_data_3.0_underc_addpca_addnmf)*0.8,replace = T)
train_data03.0underc_addpca_addnmf <- train_data_3.0_underc_addpca_addnmf[split,]
valid_data03.0underc_addpca_addnmf <- train_data_3.0_underc_addpca_addnmf[-split,]
dtrain03.0underc_addpca_addnmf <- xgb.DMatrix(data = as.matrix(train_data03.0underc_addpca_addnmf %>% select(-t,-p)),
                      label = train_data03.0underc_addpca_addnmf$p+100
                      # gamma 回归 y > 0
                        )
dvalid03.0underc_addpca_addnmf <- xgb.DMatrix(data = as.matrix(valid_data03.0underc_addpca_addnmf %>% select(-t,-p)),
                      label = valid_data03.0underc_addpca_addnmf$p+100
                        )
watchlist03.0underc_addpca_addnmf <- list(train=dtrain03.0underc_addpca_addnmf,
                  valid=dvalid03.0underc_addpca_addnmf
                  )
```

```{r}
train_data03.0underc_addpca_addnmf %>% names
```
...
library(lubridate)
xgb.DMatrix.save(
    dtrain03.0underc_addpca_addnmf
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dtrain03.0underc_addpca_addnmf.buffer',sep='_')
    )
)
xgb.DMatrix.save(
    dvalid03.0underc_addpca_addnmf
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'dvalid03.0underc_addpca_addnmf.buffer',sep='_')
    )
)
```
...
dtrain03.0underc_addpca_addnmf <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dtrain03.0underc_addpca_addnmf.buffer') %>% max
        )
    )
dvalid03.0underc_addpca_addnmf <- 
    xgb.DMatrix(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('dvalid03.0underc_addpca_addnmf.buffer') %>% max
        )
    )
```
...
xgb_hyper_grid <- 
    function(eta,nround,max_depth,min_child_weight,gamma,subsample,colsample_bytree,nfold){
        set.seed(123)
        xgb <- xgb.train(
         data=dtrain03.0underc_addpca_addnmf,
         ## 1
           eta = eta,
           nround=nround,
         ## 2
         ...
         ## 其他
           seed = 123,
           watchlist=watchlist03.0underc_addpca_addnmf,
           nfold = nfold,
           early_stopping_rounds = 50,
           nthread = 8
           )
...
set.seed(123)
start_time <- now()
xgb_base05underc_addpca_addnmf <- xgb.train(
    data=dtrain03.0underc_addpca_addnmf,
    ## 1
    eta = 0.3,
    nround=200,
...
end_time <- now()
get_log_xgb(xgb_base05underc_addpca_addnmf,start_time,end_time)
```

```{r eval=F}
xgb.save(
    xgb_base05underc_addpca_addnmf
    ,file.path(
        'data'
        ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'xgb_base05underc_addpca_addnmf.model',sep='_')
    )
)
```
...
xgb_base05underc_addpca_addnmf <- 
    xgb.load(
        file.path(
            'data'
            ,list.files('data') %>% str_subset('xgb_base05underc_addpca_addnmf.model') %>% max
        )
    )
```
...
test_data_3.0 %>% 
    ...
    mutate(
        predicition = 
            ...
            mutate(pc1 = test_pca) %>% 
            mutate(nmf = test_nmf) %>%             
            select(colnames(train_data03.0underc_addpca_addnmf %>% select(-t,-p))) %>% 
            # names
            as.matrix %>% 
            predict(xgb_base05underc_addpca_addnmf,.) %>%
            # predict(xgb_base05underc_addpca_addnmf
            #         ,newdata = train_data03.0underc_addpca_addnmf %>% 
            #             select(-t,-p) %>% 
            #             as.matrix()
            #             )
            `-`(100)
...
```{r eval=F}
xgb.importance(feature_names = colnames(dtrain03.0underc_addpca_addnmf), model = xgb_base05underc_addpca_addnmf) %>% 
    write_excel_csv(
        file.path(
            'data'
            ,paste(today() %>% str_remove_all('-') %>% str_sub(3,-1),'jiaxiang_imp_tbl.csv',sep='_')
...
```{r eval=F}
valid_data03.0underc_addpca_addnmf_train <- 
    valid_data03.0underc_addpca_addnmf %>% 
    ungroup %>% 
    # unless
    # Error in mutate_impl(.data, dots) : Column `predicition` must be length 11808 (the group size) or one, not 46571
    select(id,t,p) %>% 
    mutate(
        predicition = 
            ...
            valid_data03.0underc_addpca_addnmf %>% 
            select(
                colnames(dtrain03.0underc_addpca_addnmf)
                ) %>% 
            # setdiff(
            #     colnames(dtrain03.0underc_addpca_addnmf)
            #     ,colnames(valid_data03.0underc_addpca_addnmf)
            # )
            ungroup() %>% 
            as.matrix %>% 
            predict(xgb_base05underc_addpca_addnmf,.) %>% 
...
add2evaluation::phv_metric(
    valid_data03.0underc_addpca_addnmf_train$short_name
    ,valid_data03.0underc_addpca_addnmf_train$t
    ,valid_data03.0underc_addpca_addnmf_train$p
    ,valid_data03.0underc_addpca_addnmf_train$predicition)
```
